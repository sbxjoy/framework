...
class FwDB
{
	...
	{/*{{{*/
		...
		if(!isset(self::$_container[$key]) || !(self::$_container[$key] instanceof FwDBPDO)) {
			$final_config = array();
			...
			self::$_container[$key] = new FwDBPDO($final_config);
		}
...
class FwDBPDO
{
	...
	{/*{{{*/
		if($this->_conn == null) {
			...
			try {
				...
				throw new FwDBException($e->getMessage(), $e->getCode());
			}
	...
	{/*{{{*/
		...
		$stmt = new FwDBStatment($this->_conn->prepare($sql));
		if(is_array($params)) {
			if(!empty($params)) {
				$i = 0;
		...
		if($this->_optimize && preg_match("/^select/i", $sql)) {
			...
			$debug = $this->_debug;
			$this->setDebug(true);
			FwDBExplainResult::draw($this->getAll("explain ".$this->getBindedSql($sql, $params)));
			$this->setDebug($debug);
	...
	{/*{{{*/
		...
		if(in_array(preg_replace("/\s{2,}/", " ", strtolower($sql)), array("begin", "commit", "rollback", "start transaction", "set autocommit=0", "set autocommit=1"))) {
			throw new FwDBException("为避免操作异常，请使用包装后的事务处理接口[startTrans, commit, rollback]");
		}

		if($this->_transaction) {
			if($this->_reconnected) {
				throw new FwDBException("数据库链接已丢失!");
			} else {
				try {
					...
					throw new FwDBException($e->errorInfo[2], $e->errorInfo[1]);
				}
			}
		} else {
			try {
	...
	{/*{{{*/
		...
		if(!$is_safe) {
			throw new FwDBException("SQL语句:[$sql],存在SQL注入漏洞隐患，请改用bind方式处理或关闭sql执行safe模式.");
		}
	...
	{/*{{{*/
		if(in_array($operate, array("insert", "replace", "update"))) {
			...
			if(empty($fields)) {
				throw new FwDBException("\$record 操作数据必须使用关联数组形式");
			}
		}
	...
	{/*{{{*/
        try {
            ...
            throw new FwDBException($e->getMessage());
        }
	}/*}}}*/
	...
	{/*{{{*/
		if($this->_transaction) {
			throw new FwDBException("之前开启的事务尚未结束，事务处理不能嵌套操作!");
		}
		...
		try {
			...
			throw new FwDBException($errorInfo[2], $errorInfo[1]);
		}
	...
	{/*{{{*/
		if(!$this->_transaction) {
			throw new FwDBException("之前开启的事务已经被提交或没有开启，请仔细查看事务处理过程中的操作语句!");
		}
...
class FwDBException extends Exception
{
